...
### Публикация и картинки патча

`GET /patches/:version/publish` → `publishPatch` (async): берёт `result/changelog.json`,
убирает сырые значения/`cs`, поднимает `icon`, и **переносит используемые картинки** в бакете
в папку `pages/patches/<version>/`:

- собираются все `image`-поля, **кроме**: стандартных (`pages/patches/…`), превью героев
  (`…/heroesPreview/…`), общей innate-иконки (`…/innate_icon…`), базовых/фронтовых иконок
  (пути с ведущим `/`, напр. `/str.png`, `/health_regen.png`) и уже обрезанных (без «/»);
  поля `icon` (public фронта) не трогаются. Реальные картинки способностей (в т.ч. врождённых,
  напр. `abilities/dazzle_innate_weave.webp`) — это контент, они копируются;
- **папка пересоздаётся**: при повторной публикации `pages/patches/<version>/` в бакете сначала
  полностью очищается (`ListObjectsV2` + `DeleteObjects`), затем заливается заново — устаревших
  или переименованных картинок не остаётся (стандартные `pages/patches/standard_*` не в подпапке
  версии, их не трогает);
- каждая копируется из исходного ключа в `pages/patches/<version>/<имя>`
  (`assetIndex.copyPatchImages`, S3 `CopyObject`); имя: предмет — имя файла; **талант** — всегда
  с именем героя в начале (`warlock_8.webp`), т.к. у талантов имена — просто номера;
  **способность** — по `ability_id` (уже начинается с имени героя). Остаточные коллизии
  добиваются папкой-родителем — см. `buildPatchImageNames`/`patchImageName`;
- в `published.json` путь скопированной картинки **обрезается до этого имени**; фронт достраивает
  `pages/patches/<version>/<имя>` (`patchImageUrl` в `pages/patches/patches.utils.ts`, версия
  прокинута в `EntityBlock`/`AbilityList`/`Items`). Полные пути (превью, стандарты) — как есть.
- Без S3-конфига копирование пропускается, пути остаются полными (фронт грузит оригиналы).
...
- `heroes[]` — по каждому герою: `base` (атрибуты, плоско), `abilities` (группы), `talents`, `texts{ru,en,uk}`;
  - **дефолты `DOTAHeroBase`** (`HERO_BASE_DEFAULTS` в `renderChangelog.js`): если атрибут раньше не
    был задан у героя (diff = `added`, `old = null`), он наследовал дефолт из шаблона — генератор
    подставляет его в `old` и делает `changed`. Сейчас в карте `BaseAttackSpeed = 100`
    (`null → 105` ⇒ «с 100 до 105», а не «добавлено»); новые дефолты добавлять туда же;
