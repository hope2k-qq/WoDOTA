...
function wodatalents:talent_learn(params)
    ...
    if (playerstalents[params.PlayerID][params.attribute] or 0) < _G.line_level_info[talent_branch] then
        return
    end
    ...
    if params.attribute == "str" then
        player_system:PlayerQuestProgress(params.PlayerID, 15, 1)
        player_system:PlayerQuestProgress(params.PlayerID, 16, 1)
    elseif params.attribute == "agi" then
        player_system:PlayerQuestProgress(params.PlayerID, 37, 1)
        player_system:PlayerQuestProgress(params.PlayerID, 38, 1)
    elseif params.attribute == "int" then
        player_system:PlayerQuestProgress(params.PlayerID, 59, 1)
        player_system:PlayerQuestProgress(params.PlayerID, 60, 1)
    end
end
